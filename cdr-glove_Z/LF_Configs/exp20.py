...
def distanceImproveBeforeDisease(c):
    m=re.search(r'(improv)(.*)({{B}}.*{{A}})', get_tagged_text(candidate), flags=re.I)
    if(m):
...
def LF_induced_other(c):
    return (rule_regex_search_tagged_text(c, '{{A}}.{20,1000}-induced {{B}}', -1),distanceCD(c))
...
def LF_far_c_d(c):
    return (rule_regex_search_btw_AB(c, '.{100,5000}', -1),distanceCD(c))
...
def LF_far_d_c(c):
    return (rule_regex_search_btw_BA(c, '.{100,5000}', -1),distanceCD(c))
